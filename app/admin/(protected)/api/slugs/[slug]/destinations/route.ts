...
import type { DestinationWithClicks } from "@/lib/storage/types";
...
export async function POST(req: Request, ctx: { params: Promise<{ slug: string }> }) {
  ...
  try {
    const kv = getKv();
    const next = await kv.addDestination({ slug, name, urls });
    const destination = next.destinations[next.destinations.length - 1];
    const withClicks: DestinationWithClicks = { ...destination, clickCount: 0 };
    return NextResponse.json({ destination: withClicks }, { status: 201 });
  } catch (err) {
